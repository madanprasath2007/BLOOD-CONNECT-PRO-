...
import { Send, User, Hash, Calendar, Droplets, Activity, ClipboardList, ShieldCheck } from 'lucide-react';
...
const HospitalRequestForm: React.FC<HospitalRequestFormProps> = ({ onSubmit, hospitalName }) => {
  ...
  return (
    <div className="bg-white rounded-[2rem] border border-slate-100 shadow-xl shadow-slate-200/50 overflow-hidden animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        ...
        <div className="pt-4">
          <button type="submit" disabled={isSubmitting} className="w-full bg-red-600 text-white py-5 rounded-2xl font-bold text-sm hover:bg-red-700 transition-all shadow-xl shadow-red-200 flex items-center justify-center gap-2 group disabled:opacity-70">
            {isSubmitting ? <Activity className="w-5 h-5 animate-spin" /> : <><Send className="w-4 h-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" /> Broadcast Urgent Requirement</>}
          </button>
        </div>
      </form>
    </div>
